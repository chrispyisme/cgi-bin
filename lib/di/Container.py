...
class Container:
    ...
    def _resolve(self, abstract: str, params: Dict) -> Any:
        ...
        # If concrete is a string handler with '@', treat as callable
        if isinstance(concrete, str) and '@' in concrete:
            callable_fn = self.make_callable(concrete)
            return self._call_callable(callable_fn, params)

        # Callable factory (lambda, function, or method)
        if callable(concrete) and not inspect.isclass(concrete):
            return self._call_callable(concrete, params)
    ...
    def _build(self, concrete: Any, params: Dict) -> Any:
        ...
        # If string and contains '@', treat as callable factory and invoke it
        if isinstance(concrete, str) and '@' in concrete:
            callable_fn = self.make_callable(concrete)
            return self._call_callable(callable_fn, params)
        ...
        for name, param in sig.parameters.items():
            ...
            # 1b. SPECIAL CASE: Container reference via 'c' parameter
            # This mirrors how handlers receive the container
            if name == 'c':
                if param.kind in (param.POSITIONAL_OR_KEYWORD, param.POSITIONAL_ONLY):
                    args.append(self)
                else:
    ...
    def _call_callable(self, fn: Callable, params: Dict) -> Any:
        """
        Invoke a callable (function or bound method) with smart parameter passing.

        - If first parameter is named 'container', 'c', or 'cont', pass the container
        - Otherwise pass provided `params` as keyword args
        """
        sig = inspect.signature(fn)
        param_names = list(sig.parameters.keys())

        # If first param expects the container, pass it
        ...
        # Otherwise try to call with params as kwargs
        try:
            return fn(**(params or {}))
        except TypeError:
            # Fallbacks: try positional empty, then no-arg
            try:
                return fn()
            except TypeError as e:
                raise AppException(f"Failed to call factory {fn}: {e}") from e
    ...
    def bind(self, abstract: Any, concrete: Any, **params) -> None:
        ...
        #self._binding_params[key] = {"c": self}  # Always provide container as param for factories
    ...
    def singleton(self, abstract: Any, concrete: Any, **params) -> None:
        ...
        """
        key = self._normalize_key(abstract)
        #self._binding_params[key] = {"c": self}  # Always provide container as param for singletons
        self._bindings[key] = concrete
